...
+ P9(오프닝 익스플로러·수순 탐색기) + P10(엔진 운용 도구·콘솔 명령어)** 까지
구현되어 있다.

> 목표는 **Yixin.exe의 모든 설정·기능 이식**이며, 외관·입출력·안정화가 남았다
> (P11 — 계획서 §4 인벤토리·§5 단계 참고).
...
> 확인한다**(현재 420 테스트 통과, APK 빌드 성공). 기기에서의 화면·실서버 동작 확인은
> 여전히 사용자 몫이다.
...
P11(외관·입출력·안정화: 언어 로더, 툴바·핫키 UI, 스킨·폰트, 디버그 로그, About,
워치독·자동 재연결·포그라운드 유지·회전/태블릿 레이아웃). 상세는 계획서 §5.
