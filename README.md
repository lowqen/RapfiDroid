...
+ 보드 툴바 + P5(대국 기능) + P8(기보 I/O·리뷰·리포트·큐) + P8b(국면 증명)** 까지
구현되어 있다.

> 목표는 **Yixin.exe의 모든 설정·기능 이식**이며, 연구 도구(오프닝 익스플로러·수순
> 탐색기)와 엔진 운용 명령은 아직 미구현이다 (P9~P11 — 계획서 §4 인벤토리·§5 단계 참고).
...
> 확인한다**(현재 298 테스트 통과, APK 빌드 성공). 기기에서의 화면·실서버 동작 확인은
> 여전히 사용자 몫이다.
...
- **테스트**: `ProveTreeTest`(44)·`ProveRepositoryTest`(20) → 전체 **298개** 통과.

## 다음 (P9)

P9(오프닝 익스플로러·수순 탐색기 — 국면 키 Kotlin 포팅은 `rifdb/rif_crosscheck.py` 4중
교차검증 필수) → P10(엔진 운용) → P11(외관·안정화). 상세는 계획서 §5.
