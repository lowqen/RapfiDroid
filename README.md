...
+ 보드 툴바 + P5(대국 기능)** 까지 구현되어 있다.

> 목표는 **Yixin.exe의 모든 설정·기능 이식**이며, 기보 I/O·리뷰와 연구 도구는
> 아직 미구현이다 (P8~P11 — 계획서 §4 인벤토리·§5 단계 참고).
...
> 확인한다**(현재 175 테스트 통과, APK 빌드 성공). 기기에서의 화면·실서버 동작 확인은
> 여전히 사용자 몫이다.
...
P8(기보 I/O·리뷰·증명·큐) → P9(오프닝/수순 탐색기) → P10(엔진 운용) →
