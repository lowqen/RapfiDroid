...
전체 고지는 **[LICENSES.md](LICENSES.md)**, 앱 안에서도 볼 수 있다
(설정 ▸ ⓘ ▸ 「라이선스 전문 보기」). 요약:
...
| RenjuNet 파생 자료 | 비상업·**오프라인 전용** | **동봉·온라인 배포 금지** |

**원저작자에게 허락을 받아야 하는 구성요소는 없다** — 전부 고지 또는 소스 공개를
요구할 뿐이다. 다만 둘은 반드시 지킨다: ① 엔진 fork 저장소를 **공개**로 둘 것(GPL
위반 방지), ② RenjuNet 파생 팩을 APK·릴리스에 **절대 넣지 말 것**(원문이
"or its modifications in any website or ONLINE system" 을 금지한다).
Play 스토어는 GPLv3 와의 알려진 충돌이 있으므로 LICENSES.md §3 을 먼저 읽을 것.

### 라이선스 주의 (자료)

- 오프닝 익스플로러 데이터(`freq_data.json`/팩)는
  **RenjuNet 파생 → 재배포·스토어 배포 금지**. P3에서 사용자 기기 반입(SAF)으로 구현했고
  앱은 그 데이터를 절대 내보내지 않는다(설정 내보내기는 `settings*.txt`뿐).
- 데이터가 없어도 앱은 동작한다 — 26주형·오프닝 이름(249개)·흑5수 유불리는
  번들 자료로 나오고, 빠지는 것은 «실전 빈도·승률»뿐이다.
...
- **분석 엔진**: `FreqAnalyzer`(순수) = `freq35.rankings` 포팅. 선수/룰 필터 →
  3수 26주형 승·무·패 집계 / 5수 실전 빈도 랭킹.
- **UI**: 26주형 카드 그리드(미니보드+3색 결과막대), 5수 리스트(실전 빈도순,
  수순 검색), 선수 자동완성·룰 칩. 데이터가 없으면 26주형 카드만 뜬다.

> 5수 모양의 주형 라벨은 `classify(대표 수순)`이다. 한 모양은 여러 착수 순서로
> 도달할 수 있으므로 그 라벨은 «이 대표 수순의 주형»이라는 뜻이지 모양 자체의
> 유일한 주형은 아니다 — 그 갈래를 보여 주는 곳은 익스플로러의 환원 목록이다.

라이선스: freq는 사용자 반입·비공유(§라이선스 주의 그대로).
...
바로 `Rankings` 탭에서 26주형 확인, ③ freq 임포트로 실전 빈도 확인,
④ (P1/P2) Tailscale on + `Connect` 탭으로 실서버 왕복·좌표(y,x) 검증.

**CI (2026-08-30 추가).** `.github/workflows/build.yml` 이 push 마다 우분투 러너에서
JDK 17 로 `testDebugUnitTest` → `assembleDebug` 를 돌리고 디버그 APK 를 아티팩트로
올린다. 테스트가 APK 앞에 오는 건 동작이 깨지면 산출물이 나오지 않게 하려는 것이고,
실패하면 HTML 리포트가 함께 올라와 로컬 재현 없이 읽을 수 있다. `local.properties` 는
gitignore 되어 있으므로 러너는 자기 `ANDROID_HOME` 을 쓴다 — 즉 **JDK·SDK 가 없는
PC 에서도 검증 경로가 있다.** Actions 탭 → 실행 선택 → Artifacts 에서 `app-debug` 를
받으면 된다.
...
## 아이콘 · 고정 헤더가 목록을 굶기던 자리 — 2026-08-30

- **런처 아이콘을 사진으로 바꿨다.** 사진은 VectorDrawable 이 될 수 없어 foreground 를
  5개 밀도 PNG(108dp = 108~432px)로 굽고 기존 벡터 foreground 는 지웠다. 문제는 받은
  아트워크에 **금색 테두리가 이미 그려져 있다**는 것 — 어댑티브 아이콘은 108dp 중
  **중앙 72dp 만** 마스킹해 보여주므로, 아이콘 모양의 테두리를 그 마스크 안에 다시
  넣으면 바둑판이 보이는 영역의 절반으로 줄어든다. 그래서 아트워크를 캔버스의 **90%**
  로 놓아 테두리가 마스크가 버리는 바깥 블리드에 통째로 숨게 했다: 런처의 마스크가 곧
  테두리가 되고, 보이는 자리는 바둑판 면·금색 격자·두 돌이 채운다. 가장 좁게 자르는
  **원형 마스크로 검증**했고 두 돌 모두 온전히 들어온다. background 는 나무 그라디언트
  대신 사진의 바탕색(#060504)이다 — 패럴랙스가 가장자리를 벌릴 때 밝은 링이 새는 걸
  막는다. `launch_icon_background` 도 같은 이유로 이 값이고, 테마가 아니라 **그림의
  속성**이라 `values-night` 쌍을 잃었다.

- **설정 화면에서 위쪽 고정 항목이 아래 목록을 덮고 있었다.** 원인은 겹침이 아니라
  측정이다. `Column` 은 weight 없는 자식을 먼저 재고 **남은 것만** weight 자식에게
  준다. 헤더에 검색·칩·고급 스위치·반입 카드·파일 6줄·디버그 줄·초기화·메시지 배너가
  다 있어 약 650dp 였고, 세로에서는 목록이 실오라기, **가로에서는 0dp** 로 측정됐다.
  게다가 `Column` 은 clip 하지 않아 넘친 버튼이 목록 자리에 그대로 그려졌다 — 덮은 게
  맞다. 고정으로 남긴 건 검색 필드와 카테고리 줄뿐(그 아래 divider 가 경계), 나머지는
  전부 목록의 첫 아이템으로 내려 함께 스크롤한다. 칩은 `FlowRow`→`LazyRow` 로 바꿨다:
  8개 라벨이 영어에서 2줄, 한국어 큰 글꼴에서 3줄로 늘어나던 게 고정 블록에 상한이
  없던 진짜 이유다. 이제 화면과 무관하게 약 120dp.

- **메시지 배너는 옮기지 않고 없앴다.** `AppChrome` 이 "앱에 스낵바 하나"를 못박아 두고
  이미 4개 화면이 그걸 쓰는데 이 화면만 인라인 배너로 남아 있었고, 하필 **높이가 문자열
  길이에 달린** 유일한 헤더 요소였다. 회전이 이 수정의 시험대라 `showAbout`·`showFiles`
  와 반입 카드의 `showDetail` 은 `rememberSaveable` 로 바꿨다(카드는 이제 스크롤되어
  화면 밖으로 나가므로 `remember` 면 펼친 결과가 접힌다).

- **연결 화면도 같은 병이었다.** 엔드포인트 2필드·상태 줄·헬스 줄·콘솔 헤더·명령 줄이
  약 360dp 고정이고 콘솔만 `weight(1f)` — 가로 폰에서 콘솔이 0dp 였다. 보드 화면이 이미
  640dp 에서 두 단으로 갈라지므로 그 숫자를 `WideLayoutMin` 으로 디자인 시스템에 올리고
  두 화면이 같은 폭에서 갈라지게 했다. 왼쪽은 엔드포인트·상태, 오른쪽은 콘솔과 그
  콘솔에 넣는 명령 줄(명령은 답을 보면서 쓰는 것이라 떼지 않는다). 세로는 그대로다.

- **성능: 매번 다시 컴파일되던 정규식들.** `Regex(…)` 는 만들 때 패턴을 컴파일한다.
  `ResponseParser` 는 `bestMove` 를 이미 `private val` 로 두고 있었는데 나머지 넷
  (`\s+`, `\d+`, `-?\d+`, ABOUT 필드 문법)은 함수 안에서 만들고 있었다 — 탐색 중
  엔진이 INFO·MESSAGE 를 계속 흘리므로 **엔진 줄마다** 다시 컴파일됐다는 뜻이다.
  `parseThinking` 은 같은 문자열을 두 번 split 해 한쪽 크기와 비교하고 있었고,
  연결 화면의 `isCoordinate` 는 **콘솔 줄마다·리컴포지션마다** 패턴을 새로 지었다.
  전부 상수로 올렸다. 문법은 그대로 — `ResponseParserTest` 가 CI 에서 이를 지킨다.

- **`SettingsUiState.visible` 을 `get()` → `by lazy` 로.** 67개 스펙을 돌며 최대 4번씩
  대소문자 무시 `contains` 를 하는데 화면이 조합마다 두 번 읽는다(부제의 개수, 그리고
  목록). 검색창에 한 글자 칠 때마다 전체 필터가 두 번 돌고 있었다. 상태는 불변이고
  변경마다 새 인스턴스가 생기므로 인스턴스에 캐시하는 건 다시 계산하는 것과 정확히
  같은 신선도다.

- **`Stepper` 3벌 → 1벌.** 증명·리뷰 화면이 자간·U+2212 빼기 기호까지 똑같은 사본을
  하나씩 갖고 있었고 다른 건 "한 번에 얼마씩 움직이나" 하나뿐이었다. 그게 `step`
  파라미터가 되어 디자인 시스템으로 갔다(리뷰는 1씩, 증명은 예산이 커지면 보폭도
  커진다 — `budgetStep`). 데이터베이스 화면 것은 **일부러 남겼다**: 문장 사이에 끼어
  들어가는(«저장 간격 [−|5|+] 분») 작은 컨트롤이라 성격이 다르고, 억지로 합치면 둘을
  구분하려고 스타일 플래그를 다는 더 나쁜 코드가 된다. 같은 이유로 이름만 같고 하는
  일이 다른 `Section` 3벌도 그대로 뒀다.

- **CI 가 테스트를 지나치고 있었다.** 단위 테스트 클래스가 43개인데 워크플로는 빌드만
  했다 — 초록이 "컴파일된다"는 뜻뿐이었다. 위의 파서 리팩토링이 바로 그 테스트가
  지켜야 할 것이라 `testDebugUnitTest` 를 APK 앞에 넣었다. RenjuNet 자료가 필요한
  테스트는 `assumeTrue` 로 스스로 건너뛰므로 러너에서 안전하다.

## 승률순 랭킹 · 필터 시트가 버튼을 자르던 자리 — 2026-08-30

랭킹 탭이 답하지 못하던 질문 하나와, 필터를 쓰면 «적용»을 누를 수 없던 문제 하나.

- **필터에 «색»이 생겼다(흑/백/양쪽).** 한 가지 선택이 두 가지 일을 하는 건 그게 사실
  같은 질문이기 때문이다. 선수를 고른 상태면 **그 선수가 그 색으로 둔 대국만** 센다 —
  «Alice가 흑일 때»는 «Alice»와 다른 집합이고, 백 대국이 섞이면 «흑으로 어느 주형에
  강한가»라는 질문 자체가 성립하지 않는다. 동시에 새 승률 정렬이 **어느 쪽 승률을
  읽을지**를 정한다. 선수를 안 골랐으면 색만으로는 아무 대국도 걸러지지 않는다(모든
  대국에 흑과 백이 다 있다) — 그때는 정렬 기준으로만 쓰인다.

- **승률순 정렬은 필터가 뭔가를 좁혔을 때만 나타난다.** 요청대로다. 전체 대국에 대한
  «가장 성적 좋은 주형»은 렌주에 대한 사실이지 사용자가 물어본 것이 아니다. 3수 탭은
  번호순/실전순/승률순, 5수 탭은 실전순/승률순.

- **정렬 키는 생승률이 아니라 95% Wilson 신뢰구간의 하한이다.** 한 선수를 한 룰로
  좁히면 대부분의 주형에 대국이 몇 판씩만 남고, 거기서 1승 1판은 100%다 — 생승률로
  정렬하면 «한 번 두고 이긴 주형»이 1등이 되어 아무 질문에도 답하지 못한다. Wilson 은
  «이만큼의 판수로 참 승률이 얼마나 낮을 수 있나»를 묻기 때문에 1판 100%는 0.21,
  20판 55%는 0.34 로 뒤가 앞선다. 카드에는 **실제 승률과 실제 판수**가 그대로 나오고,
  순서가 그 숫자와 어긋나 보이는 건 버그가 아니므로 화면이 한 줄로 그렇게 말한다.

- **3수 정렬 칩이 자기 줄로 내려왔다.** 원래는 두 상태를 오가는 칩 하나여서 直/間 칩
  옆에 들어갔는데, 이름 붙은 정렬이 셋이 되면 들어가지 않는다. 그리고 그 자리는
  `Row` 라 줄바꿈이 없어서 — 폰에서 승률 칩이 화면 밖으로 밀려났을 것이다.

- **필터를 좁히지 않게 되면 정렬도 따라 돌아온다.** 선수를 빼거나 룰을 끄거나 데이터를
  해제하면 승률 정렬의 칩이 사라지는데, 목록만 그 순서로 남으면 아무도 고르지 않았고
  되돌릴 수도 없는 정렬이 된다. 필터를 줄일 수 있는 인텐트는 전부 정렬을 다시 검사한다.

- **필터 시트가 버튼을 자르고 있었다.** 시트 내용이 스크롤 없는 `Column` 하나여서,
  선수를 고르면 제안 목록 180dp 와 선택 칩 줄이 더해지며 화면을 넘어갔고 «적용»과
  «필터 초기화» 가 아래로 잘려 **누를 방법이 없었다**. 내용은 이제 스크롤하고
  (`weight(1f, fill = false)` — 내용이 짧으면 시트도 짧게 유지된다) 두 버튼은 그 아래
  구분선 뒤에 고정된다.

- **이 PC 에서는 앱을 띄울 수 없으므로 단정하지 않고 테스트로 고정했다**(CI 가 돌린다):
  색 필터가 두 랭킹에 미치는 영향, 색만으로는 아무것도 걸러지지 않는다는 것,
  그리고 랭킹 점수의 순서 보장(1판 100% < 20판 55%, 같은 승률이면 판수 많은 쪽이 위).
