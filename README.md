...
+ 보드 툴바 + P5(대국 기능) + P8(기보 I/O·리뷰·리포트·큐)** 까지 구현되어 있다.

> 목표는 **Yixin.exe의 모든 설정·기능 이식**이며, 국면 증명(P8b)과 연구 도구는
> 아직 미구현이다 (P8b~P11 — 계획서 §4 인벤토리·§5 단계 참고).
...
> 확인한다**(현재 233 테스트 통과, APK 빌드 성공). 기기에서의 화면·실서버 동작 확인은
> 여전히 사용자 몫이다.
...
- **테스트**: `GameFileTest`(13)·`MoveQualityTest`(22)·`ReportFormatsTest`(9)·
  `ReviewRepositoryTest`(14) → 전체 **233개** 통과.

## 다음 (P8b)

P8b(국면 증명 — AND/OR 탐색이 운영 DB에 쓰기 때문에 P8에서 분리) → P9(오프닝/수순
탐색기) → P10(엔진 운용) → P11(외관·안정화). 상세는 계획서 §5.
